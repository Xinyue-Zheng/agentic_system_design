...
class KPIQueryBuilder:
    ...
    def build(self, start, end, kpi_names,
              time_granularity="day", spatial_level="cell",
              enodebs=None, cells=None):
        """Build one parameterized query over an adaptive scope.

        Scope (provide at least one; both = intersection):
          - enodebs : whole-site intent. Single equality/IN on the site
                      column -- cheapest filter, likely hits partition
                      pruning. Use when querying all cells of site(s).
          - cells   : precise subset intent (globally unique cell ids),
                      e.g. neighbor cells straight from a coverage scan.

        Args:
            start, end: time window (end exclusive). Values are BOUND at
                execution time: pass the returned params dict to
                cursor.execute(sql, params) -- the SQL text itself never
                contains literal values (injection safety + plan reuse).
            kpi_names: list of KPI names (display_name or kpi_id, any case).
            time_granularity: 'day' | 'hour' | '15min'.
            spatial_level: 'cell'   -> one row per cell per time bucket;
                           'enodeb' -> grouped by owning site (num & den
                                       summed across each site's cells
                                       before dividing).
        ...
        if isinstance(enodebs, str):
            enodebs = [enodebs]
        if isinstance(cells, str):
            cells = [cells]
        if not enodebs and not cells:
            raise ValueError("scope required: provide enodebs and/or cells")
...
if __name__ == "__main__":
    ...
    # Execution pattern:  df = run_query(sql, params)   # params bound by connector
    b = KPIQueryBuilder()
    try:
        # 1. whole-site intent: enodebs scope, aggregated per site
        sql, params = b.build(
            start="2026-05-01", end="2026-05-29",
            kpi_names=["RRC Connected Users"],
            time_granularity="day", spatial_level="enodeb",
            enodebs="111111",
        )
        print(sql, "\n", params, "\n", "-" * 50)

        # 2. precise-subset intent: neighbor cells from a coverage scan
        sql, params = b.build(
