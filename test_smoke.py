...
RUN_LIVE = False
TEST_CELLS = ["PLACEHOLDER_CELL_1"]   # cell ids are globally unique
...
def stage_a_dry_run(builder: KPIQueryBuilder) -> None:
    ...
    try:
        sql, params = builder.build(
            TEST_START, TEST_END, kpis, cells=TEST_CELLS,
    ...
    try:
        builder.build(TEST_START, TEST_END, ["rrc conected users"], cells=TEST_CELLS)
    except UnknownKPIError as e:
