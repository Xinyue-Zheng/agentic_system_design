#!/usr/bin/env python3
"""Two countryside maps WITHOUT a real map background (plain coordinate canvas).

Same as countryside_map.py except the OSM *tile* basemap is not fetched or
drawn -- figures are plotted on a plain background with a lat/lon grid instead
of real map imagery. All computation (population clustering, OSM feature
fetching, amenity+residential settlement clustering/merging) is unchanged.
...
Valley (Monterey County) and writes TWO figures:
  ...
  1. pop_density_map_nobasemap.png  -- population-density heatmap
     (usa_pd_2018_1km.tif) on a plain background, with the dense pockets
     circled; their lat-lon ranges are printed to the console.
  2. osm_features_map_nobasemap.png -- OSM interest features on a plain
     background: highway / railway (lines), landuse / leisure / building
     (polygons), amenity (points). Amenity POIs and residential landuse
     parcels are clustered together into "settlement" boxes wherever they
     sit close to each other, and each box's lat-lon range is annotated
     *on the map*.

Everything is still drawn in Web Mercator (EPSG:3857) internally -- purely so
the geometry code is identical to countryside_map.py -- even though there's
no tile image to align it with.
...
OUT_POP = "pop_density_map_nobasemap.png"
OUT_OSM = "osm_features_map_nobasemap.png"

# background color for the plain (non-basemap) canvas
CANVAS_FACECOLOR = "#f4f4f0"
...
_HEADERS = {"User-Agent": USER_AGENT}  # Overpass rejects requests without one
...
def setup_axes(ax, west, south, east, north, title):
    """Plain canvas version: no tile image, just a flat background + lat/lon grid."""
