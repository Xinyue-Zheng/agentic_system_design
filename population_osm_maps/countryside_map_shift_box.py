...
def merc_inv(x, y):
    """Inverse Web Mercator: meters -> (lon, lat) degrees."""
...
def box_road_rail_kinds(feats, lo_lon, hi_lon, lo_lat, hi_lat):
    """Which of {"highway", "railway"} have at least one vertex inside the box."""
    kinds = set()
    for geom, _ref, _cls in feats.get("highway", []):
        if any(lo_lon <= p["lon"] <= hi_lon and lo_lat <= p["lat"] <= hi_lat for p in geom):
            kinds.add("highway")
            break
    for geom in feats.get("railway", []):
        if any(lo_lon <= p["lon"] <= hi_lon and lo_lat <= p["lat"] <= hi_lat for p in geom):
            kinds.add("railway")
            break
    return kinds
...
def figure_osm(west, south, east, north, feats):
    ...
    if OSM_LAYERS["highway"] and feats["highway"]:
        segs = [ring_to_merc(g) for g, _ref, _cls in feats["highway"] if len(g) >= 2]
    ...
    mid_box = None  # the settlement box containing (CENTER_LAT, CENTER_LON)
    mid_idx = None
    for i, b in enumerate(boxes, 1):
        ...
        if y1 > yt - (yt - yb) * 0.12:
            anchor_y, dy_off, va = y0, -6, "top"
        else:
            anchor_y, dy_off, va = y1, 6, "bottom"
        ax.annotate(label, xy=((x0 + x1) / 2, anchor_y), xytext=(0, dy_off),
        ...
        if lo_lon <= CENTER_LON <= hi_lon and lo_lat <= CENTER_LAT <= hi_lat:
            mid_box, mid_idx = (lo_lon, hi_lon, lo_lat, hi_lat), i

    if mid_box is None and boxes:
        # fall back to whichever box's centroid is nearest the configured center
        def _dist(b):
            clon, clat = (b["lo_lon"] + b["hi_lon"]) / 2, (b["lo_lat"] + b["hi_lat"]) / 2
            return (clon - CENTER_LON) ** 2 + (clat - CENTER_LAT) ** 2
        nearest = min(boxes, key=_dist)
        mid_idx = boxes.index(nearest) + 1
